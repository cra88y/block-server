...
func RpcClaimProgressionReward(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, payload string) (string, error) {
	...
	_, progWrite, err := PrepareProgressionUpdate(ctx, nk, logger, userID, progressionKey, req.ItemID, func(prog *ItemProgression) error {
		...
		// Mark the level as claimed with timestamp
		state.Status = "claimed"
		state.ClaimedAt = time.Now().UnixMilli()
		prog.TierStates[lvlStr] = state
		...
		// Apply ability and sprite unlocks directly into this state update from the typed mutations struct
		prog.UnlockedAbilityIndices = append(prog.UnlockedAbilityIndices, mutations.GrantedAbilities...)
		prog.UnlockedSpriteIndices = append(prog.UnlockedSpriteIndices, mutations.GrantedSprites...)
	...
	// Rebuild the progression unlocks payload correctly using the exact mutations granted
	var unlocks []notify.ProgressionUnlock
	if len(mutations.GrantedAbilities) > 0 {
		var u32Abilities []uint32
		for _, a := range mutations.GrantedAbilities {
			u32Abilities = append(u32Abilities, uint32(a))
		}
		unlocks = append(unlocks, notify.ProgressionUnlock{
			System:  req.ItemType,
			ItemID:  req.ItemID,
			Type:    "ability",
			Indices: u32Abilities,
		})
	}
	if len(mutations.GrantedSprites) > 0 {
		unlocks = append(unlocks, notify.ProgressionUnlock{
			System:  req.ItemType,
			ItemID:  req.ItemID,
			Type:    "sprite",
			Indices: mutations.GrantedSprites,
		})
	}

	if pending.Payload == nil {
		pending.Payload = notify.NewRewardPayload("claim_reward")
	}
	...
	pending.Payload.Progression.UpdatedTierStates = map[string]notify.TierState{
		strconv.Itoa(req.Level): updatedTierState,
	}
...
func RpcClaimAllProgressionRewards(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, payload string) (string, error) {
	...
	// Read current progression first to know what levels to claim
	...
	for lvlStr, state := range prog.TierStates {
		if state.Status == "unclaimed" {
			if lvl, parseErr := strconv.Atoi(lvlStr); parseErr == nil {
				levelsToClaim = append(levelsToClaim, lvl)
			}
		}
	}
	...
	for _, level := range levelsToClaim {
		...
		allMutations.GrantedAbilities = append(allMutations.GrantedAbilities, mutations.GrantedAbilities...)
		allMutations.GrantedSprites = append(allMutations.GrantedSprites, mutations.GrantedSprites...)
	...
	// Prepare progression update to claim all unclaimed rewards and apply all accumulated unlocks
	_, progWrite, err := PrepareProgressionUpdate(ctx, nk, logger, userID, progressionKey, req.ItemID, func(prog *ItemProgression) error {
		updatedTierStates = make(map[string]notify.TierState)
		for _, lvl := range levelsToClaim {
			lvlStr := strconv.Itoa(lvl)
			if state, exists := prog.TierStates[lvlStr]; exists && state.Status == "unclaimed" {
				state.Status = "claimed"
				state.ClaimedAt = time.Now().UnixMilli()
				prog.TierStates[lvlStr] = state
				updatedTierStates[lvlStr] = notify.TierState{
					Status:     state.Status,
					UnlockedAt: state.UnlockedAt,
					ClaimedAt:  state.ClaimedAt,
				}
		...
		prog.UnlockedAbilityIndices = append(prog.UnlockedAbilityIndices, allMutations.GrantedAbilities...)
		prog.UnlockedSpriteIndices = append(prog.UnlockedSpriteIndices, allMutations.GrantedSprites...)
	...
	var unlocks []notify.ProgressionUnlock
	if len(allMutations.GrantedAbilities) > 0 {
		var u32Abilities []uint32
		for _, a := range allMutations.GrantedAbilities {
			u32Abilities = append(u32Abilities, uint32(a))
		}
		unlocks = append(unlocks, notify.ProgressionUnlock{
			System:  req.ItemType,
	...
	if len(allMutations.GrantedSprites) > 0 {
		unlocks = append(unlocks, notify.ProgressionUnlock{
			System:  req.ItemType,
			ItemID:  req.ItemID,
			Type:    "sprite",
			Indices: allMutations.GrantedSprites,
		})
	}

	if pending.Payload == nil {
		pending.Payload = notify.NewRewardPayload("claim_all_rewards")
	}
	if pending.Payload.Progression == nil {
		pending.Payload.Progression = &notify.ProgressionDelta{}
	}
	...
	if len(updatedTierStates) > 0 {
		pending.Payload.Progression.UpdatedTierStates = updatedTierStates
	}
