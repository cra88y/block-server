...
func RpcClaimProgressionReward(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, payload string) (string, error) {
	...
	// Strip out any conflicting progression writes generated by PrepareLevelRewards
	if levelRewards != nil && len(levelRewards.StorageWrites) > 0 {
		var filteredWrites []*runtime.StorageWrite
		for _, w := range levelRewards.StorageWrites {
			if w.Collection != storageCollectionProgression {
				filteredWrites = append(filteredWrites, w)
			}
		}
		levelRewards.StorageWrites = filteredWrites
	}
	...
	_, progWrite, err := PrepareProgressionUpdate(ctx, nk, logger, userID, progressionKey, req.ItemID, func(prog *ItemProgression) error {
		...
		// Apply ability and sprite unlocks directly into this state update
		if levelRewards != nil && levelRewards.Payload != nil && levelRewards.Payload.Progression != nil {
			for _, unlock := range levelRewards.Payload.Progression.Unlocks {
				if unlock.Type == "ability" {
					for _, idx := range unlock.Indices {
						prog.UnlockedAbilityIndices = append(prog.UnlockedAbilityIndices, int32(idx))
					}
				} else if unlock.Type == "sprite" {
					for _, idx := range unlock.Indices {
						prog.UnlockedSpriteIndices = append(prog.UnlockedSpriteIndices, uint32(idx))
					}
				}
			}
		}
...
func RpcClaimAllProgressionRewards(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, payload string) (string, error) {
	...
	_, progWrite, err := PrepareProgressionUpdate(ctx, nk, logger, userID, progressionKey, req.ItemID, func(prog *ItemProgression) error {
		...
		if pending.Payload != nil && pending.Payload.Progression != nil {
			for _, unlock := range pending.Payload.Progression.Unlocks {
				if unlock.Type == "ability" {
					for _, idx := range unlock.Indices {
						prog.UnlockedAbilityIndices = append(prog.UnlockedAbilityIndices, int32(idx))
					}
	...
	if pending.Payload != nil && len(updatedTierStates) > 0 {
		if pending.Payload.Progression == nil {
			pending.Payload.Progression = &notify.ProgressionDelta{}
		}
